...
> ⚠️ **2026-08-26부터 `.wiki/`는 로컬이 아니라 중앙 RAG 볼트에 있습니다** — 이 문서의 모든
> `.wiki/...` 링크는 실제로는 `D:\obsidian-storage\project-rag\ai-blogging\...`(환경변수
> `PROJECT_RAG_PATH` 우선) 기준이다. 로컬 리포지토리의 `.wiki/`엔 안내 스텁만 남아있다 — §6 참고.
...
## 4. `.wiki/` vs `report/` — 지식 관리와 관리자 보고의 분리 (2026-08-22 추가, 2026-08-26 경로 갱신)

- **`.wiki/`**: Agent가 관리하는 지식 저장소. 세션 이력, 규칙, 스킬 관련 데이터의 유지관리·lint를 여기서
  수행한다(전문/근거/코드 대조 자료 — 상세하고 길어도 됨). SSOT는 여전히 `.wiki/rules/*.md`. (2026-08-26:
  이 폴더는 원래 `wiki/`였다가 §6의 중앙 RAG 스키마와 합쳐지면서 `.wiki/`로 이름이 바뀌었다 — 아래 §6
  참고, 이제 별도의 `wiki/` 폴더는 없다.)
...
## 5. 세션 핸드오프 위치 — 중앙 볼트의 `Session_Handoff.md` (2026-08-26, 세 번 이전)

큐레이션된 최신 작업 상태(진행 상황, 남은 작업, 다음 에이전트를 위한 메모)의 SSOT는
`D:\obsidian-storage\project-rag\ai-blogging\Session_Handoff.md`(환경변수 `PROJECT_RAG_PATH` 우선)
다. `.agent/session-handoff.md`는 이 경로를 가리키는 리다이렉트 스텁만 남아있다(다른 AI 툴의 고정
경로 관행 호환용) — 실제 읽기/쓰기는 중앙 볼트의 `Session_Handoff.md`에서 할 것. 같은 폴더의
`Session_Index.md`(세션 원본 아카이브 인덱스)와는 별개 문서다. (`.agent/`→`wiki/`→`.wiki/`→중앙
볼트 순으로 같은 날 세 번 이전됐다 — §6 참고.)

## 6. 중앙 RAG 지식 체계 연동 (2026-08-26, 3단계로 완료)

사용자가 여러 프로젝트에서 공통으로 쓰는 **Human-AI Coworking 표준**
(`CENTRAL_RAG_PROJECT_SCHEMA.md`, 원본은 다른 프로젝트 `elegant-pascal`에 있음)을 이 프로젝트에도
연동했다. 같은 날 세 단계로 진행됨:

1. **스키마 신설**: 기존 `wiki/`(1~5절, rules/session 아카이브 등)와 별개로 `.wiki/03-schemas/`
   (db-schemas.md/api-contracts.md), `05-direction/`(roadmap-goals.md/domain-mindmap.md),
   `06-reports/{review-inbox,digests}/`를 신설.
2. **`wiki/` → `.wiki/` 통합**: 사용자 지시로 **`wiki/`의 모든 내용을 `.wiki/`로 옮겨 하나로
   합쳤다** — 1~5절이 말하는 지식 저장소와 1단계의 중앙 RAG 스키마가 `.wiki/` 하나에 공존(내부
   상대 구조는 옛 `wiki/`와 100% 동일 보존 — `rules/`, `templates/`, `sessions/`, `theme/`,
   `Session_Handoff.md` 등).
3. **`.wiki/` → 중앙 볼트 물리 이관**: `.wiki/` 전체를 `D:\obsidian-storage\project-rag\ai-blogging\`
   (환경변수 `$PROJECT_RAG_PATH`가 있으면 그쪽 우선)로 옮겼다. **이제 이 리포지토리 안에는 지식
   파일 원본이 없다** — 로컬 `.wiki/`엔 안내 스텁(`README.md`)만 남아있고, 위 링크들의 실제 대상은
   전부 중앙 볼트 경로 기준이다(본문 최상단 경고 참고).

- **중앙 RAG 볼트**: `project-rag/ai-blogging/`이 이 프로젝트 몫이다. 원래 여기 있어야 할 범용
  규칙/스킬(`01-global-rules/`, `02-global-skills/`)은 볼트 전체에 아직 하나도 없음(다른
  프로젝트들도 마찬가지) — "중앙 볼트 참조"는 그 부분에 한해 아직 빈 참조다.
- **로컬 프로젝트 고유 지식(물리적으로는 중앙 볼트 안)** — 1단계에서 신설된 중앙RAG 스키마 3종:
  `.wiki/03-schemas/db-schemas.md`(관계형 DB는 없음, 실제 영속 저장소를 Pydantic 모델과 코드
  대조로 문서화), `.wiki/03-schemas/api-contracts.md`(CLI+외부 Blogger/Notion API 계약),
  `.wiki/05-direction/{roadmap-goals,domain-mindmap}.md`, `.wiki/06-reports/{review-inbox,digests}/`
  (Zero-Inbox 리포트 버퍼).
- **코드 경로도 함께 갱신됨**: `src/core/paths.py::central_knowledge_root()`가 이 경로 해석의
  SSOT(환경변수 `PROJECT_RAG_PATH` 우선, 기본값 고정 볼트 경로) — `article_template_path`(`main.py
  new`가 매번 읽는 실제 런타임 의존 파일)가 이걸 쓴다. 세션 아카이브 유지보수 도구 6종
  (`apply_session_backlinks.py`, `lint_session_backlinks.py`, `split_session_history.py`,
  `build_session_backlink_index.py`, `build_knowledge_graph.py`, `archive_session_log.py`)도
  동일 로직을 각자 인라인으로 갖고 있다. `main.py new` 실제 실행 + 소스/목적지 `diff -rq` 일치
  확인으로 검증 완료(2026-08-26).
- **`CLAUDE.md`/`.cursorrules`(신규)**: 둘 다 이 `AGENTS.md`를 그대로 상속하도록 첫 줄에서 임포트만
  한다 — 독자적인 규칙을 추가하지 않는다.
