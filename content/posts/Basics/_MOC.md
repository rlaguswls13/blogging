...
- [[java-compiled-or-interpreted-language|자바는 컴파일 언어인가, 인터프리터 언어인가 — 바이트코드 기준으로 정리]]
  - → [[java-classloader-parent-delegation-model|Java ClassLoader — 클래스 로딩 과정과 계층 구조(Bootstrap/Platform/App)]] *(Advanced)*
  - → [[java-generics-type-erasure-wildcard-pecs|Java 제네릭(Generics) — 타입 소거(Type Erasure)와 와일드카드(? extends/? super)]]
  - → [[jvm-heap-stack-metaspace-gc-basics|JVM 메모리 영역(Heap, Stack, Metaspace) 구조와 Garbage Collection(GC) 기본 동작 원리]]
- [[java-stream-api-lazy-evaluation-common-mistakes|Java Stream API — 중간/최종 연산과 지연 평가, 실무에서 자주 하는 실수]]
...
- [[java-string-pool-immutability-stringbuilder-stringbuffer-performance|Java String Pool과 불변성 — StringBuilder/StringBuffer 성능 차이를 직접 실측하다]]
  - → [[java-compiled-or-interpreted-language|자바는 컴파일 언어인가, 인터프리터 언어인가 — 바이트코드 기준으로 정리]]
  - → [[java-equals-hashcode-contract-guide|Java equals()/hashCode() 계약 — 왜 함께 오버라이드해야 하는가]]
  - → [[jvm-heap-stack-metaspace-gc-basics|JVM 메모리 영역(Heap, Stack, Metaspace) 구조와 Garbage Collection(GC) 기본 동작 원리]]
- [[java-checked-vs-unchecked-exception-spring-dataaccessexception|Java 예외 처리 전략 — Checked vs Unchecked Exception, 언제 무엇을 쓸 것인가]]
  - → [[solid-principles-java-guide|SOLID 원칙이란 무엇인가 — Java 기준 객체지향 설계 5대 원칙과 실전 예시]]
  - → [[spring-ioc-di-constructor-injection|Spring IoC(제어의 역전)와 DI(의존성 주입)의 명확한 정의: 왜 생성자 주입(Constructor Injection)을 선택해야 하는가]] *(Advanced)* *(허브)*
- [[java-collections-list-set-map-guide|Java 컬렉션 프레임워크 입문 — List vs Set vs Map, 언제 무엇을 써야 하는가]]
  - → [[jvm-heap-stack-metaspace-gc-basics|JVM 메모리 영역(Heap, Stack, Metaspace) 구조와 Garbage Collection(GC) 기본 동작 원리]]
  - → [[os-process-vs-thread|OS 프로세스(Process) vs 쓰레드(Thread)의 메모리 구조 차이와 컨텍스트 스위칭 원리]] *(Advanced)*
- [[java-equals-hashcode-contract-guide|Java equals()/hashCode() 계약 — 왜 함께 오버라이드해야 하는가]]
  - → [[java-collections-list-set-map-guide|Java 컬렉션 프레임워크 입문 — List vs Set vs Map, 언제 무엇을 써야 하는가]]
  - → [[solid-principles-java-guide|SOLID 원칙이란 무엇인가 — Java 기준 객체지향 설계 5대 원칙과 실전 예시]]
- [[java-generics-type-erasure-wildcard-pecs|Java 제네릭(Generics) — 타입 소거(Type Erasure)와 와일드카드(? extends/? super)]]
  - → [[java-collections-list-set-map-guide|Java 컬렉션 프레임워크 입문 — List vs Set vs Map, 언제 무엇을 써야 하는가]]
  - → [[jvm-heap-stack-metaspace-gc-basics|JVM 메모리 영역(Heap, Stack, Metaspace) 구조와 Garbage Collection(GC) 기본 동작 원리]]
- [[oop-four-pillars-java-guide|객체지향 프로그래밍 4대 특성 — 캡슐화, 상속, 다형성, 추상화의 Java 실전 적용]]
  - → [[java-equals-hashcode-contract-guide|Java equals()/hashCode() 계약 — 왜 함께 오버라이드해야 하는가]]
  - → [[solid-principles-java-guide|SOLID 원칙이란 무엇인가 — Java 기준 객체지향 설계 5대 원칙과 실전 예시]]
...
- [[solid-principles-java-guide|SOLID 원칙이란 무엇인가 — Java 기준 객체지향 설계 5대 원칙과 실전 예시]]
  - → [[gof-14|GoF 핵심 14가지 디자인 패턴 분석 및 개별 포스트 인덱스 가이드]] *(허브)*
  - → [[spring-ioc-di-constructor-injection|Spring IoC(제어의 역전)와 DI(의존성 주입)의 명확한 정의: 왜 생성자 주입(Constructor Injection)을 선택해야 하는가]] *(Advanced)* *(허브)*
...
- [[mvc-msa-architecture-evolution|MVC 패턴은 왜 여전히 중요한가 — 모놀리식 MVC에서 현대 MSA 지향 개발로의 아키텍처적 연결고리]]
  - → [[mvc|MVC 패턴: 자판기에서 동적 웹 서비스까지의 진화 및 아키텍처 분석]]
  - → [[spring-ioc-di-constructor-injection|Spring IoC(제어의 역전)와 DI(의존성 주입)의 명확한 정의: 왜 생성자 주입(Constructor Injection)을 선택해야 하는가]] *(Advanced)* *(허브)*
- [[spring-bean-lifecycle-postconstruct-beanpostprocessor-order|Spring Bean 생명주기(Lifecycle) — 컨테이너가 Bean을 생성·초기화·소멸시키는 전 과정]]
  - → [[spring-ioc-di-constructor-injection|Spring IoC(제어의 역전)와 DI(의존성 주입)의 명확한 정의: 왜 생성자 주입(Constructor Injection)을 선택해야 하는가]] *(Advanced)* *(허브)*
  - → [[spring-webflux-reactor-schedulers-troubleshooting|Spring WebFlux / Project Reactor 스레드 모델과 Schedulers 비동기 트러블슈팅]] *(Advanced)*
- [[spring-transactional-propagation-requires-new-vs-nested|Spring @Transactional 전파 옵션(Propagation) — REQUIRES_NEW vs NESTED 실전 차이]]
  ...
  - → [[spring-aop-proxy-jdk-dynamic-proxy-vs|Spring AOP(관점 지향 프로그래밍)와 프록시(Proxy) 아키텍처: JDK Dynamic Proxy vs CGLIB 기술 분석]] *(Advanced)*

## REST

- [[rest-api-design-principles-http-methods-status-codes|REST API 설계 기본 원칙 — HTTP 메서드와 상태 코드로 보는 RESTful 아키텍처]]
...
- [[sql-basics-select-insert-update-delete-join|SQL 기본 문법 입문 — SELECT/INSERT/UPDATE/DELETE와 JOIN, WHERE, GROUP BY 실전 예시]]
  - → [[mysql-innodb-btree-covering-index|MySQL InnoDB B+Tree 인덱스 내부 구조 및 커버링 인덱스(Covering Index) 성능 튜닝 레시피]] *(Advanced)*
  - → [[rdbms-1-open-source-rdbms-mysql-vs|[RDBMS 깊이 읽기 #1] Open Source RDBMS 대표주자: MySQL vs MariaDB vs PostgreSQL 기술 비교]]
  - → [[sql-vs-nosql-sqlnosql-acid-vs-base-cap|[SQL vs NoSQL] 데이터베이스 기초와 패러다임 비교: SQL/NoSQL 정의, 탄생 배경, ACID vs BASE, CAP 정리]]
...
## 🔗 허브: GoF 핵심 14가지 디자인 패턴 분석 및 개별 포스트 인덱스 가이드 (19개 글이 참조)
...
- [[gof-14|GoF 핵심 14가지 디자인 패턴 분석 및 개별 포스트 인덱스 가이드]]
  ...
  - ← [[gof-8-proxy-pattern-java|[GoF 디자인 패턴] 8. 프록시 패턴 (Proxy Pattern) 개념과 Java 실전 예시]]
  - ← [[gof-9-composite-pattern-java|[GoF 디자인 패턴] 9. 컴포지트 패턴 (Composite Pattern) 개념과 Java 실전 예시]]
  - ← [[sync-vs-async-blocking-vs-non-blocking|동기(Sync) vs 비동기(Async) & 블로킹(Blocking) vs 논블로킹(Non-blocking)의 명확한 정의]] *(Advanced)*

## 미연결 (백링크 없음 또는 아직 대상 없음)

- [[java-optional-null-safety-anti-patterns|Java Optional — null 안전성 올바른 사용법과 안티패턴]]
