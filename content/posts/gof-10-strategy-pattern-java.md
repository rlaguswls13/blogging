...
특정 작업을 수행하는 알고리즘이나 비즈니스 규칙이 여러 개 존재할 때, 이를 하나의 클래스 내에 수많은 if-else 블록으로 하드코딩하면 코드가 거대해지고 유지보수가 불가능해집니다. 새로운 정책이 추가될 때마다 기존 코드를 수정해야 하므로 OCP(개방-폐쇄 원칙)를 위배하게 됩니다.

### Section 2: 해결책 및 동작 메커니즘

전략(Strategy) 패턴은 실행 중에 알고리즘(전략)을 선택할 수 있게 해주는 행위 패턴입니다. 각 알고리즘을 캡슐화된 클래스로 분리하고 공통 인터페이스를 구현하게 하여, 클라이언트(Context)의 코드 변경 없이도 전략을 동적으로 교체할 수 있습니다.
