...
- [OS 프로세스(Process) vs 쓰레드(Thread)의 메모리 구조 차이와 컨텍스트 스위칭 원리](../../content/posts/os-process-vs-thread.md)
- [JVM 메모리 영역(Heap, Stack, Metaspace) 구조와 Garbage Collection(GC) 기본 동작 원리](../../content/posts/jvm-heap-stack-metaspace-gc-basics.md)