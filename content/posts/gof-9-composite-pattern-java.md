...
폴더 안에 파일이 들어갈 수도 있고 또 다른 폴더가 들어갈 수도 있지만, 사용자는 신경 쓰지 않고 우클릭 후 똑같이 '���성(용량 보기)' 버튼을 누릅니다.
...
## 3. 컴포지트 패턴의 실무 주의점 및 트레이드오프
      ...
      공통 인터페이스에 `add()`나 `remove()` 같은 자식 추가 메서드를 정의하면 단일 파일(Leaf) 객체에서는 사용할 수 없어 예외를 던져야 하고, 반대로 복합 객체(Directory)에만 정의하면 ���모 타입으로 다룰 때 형변환이 필요한 트레이드오프가 생깁니다.
