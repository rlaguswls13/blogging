...
tags: ["DLQ","Event-Driven","Kafka","Microservices","Resilience","Retry Pattern","Software Architecture"]
---
...
- [1. 서론: 이벤트 기반 분산 시스템에서 메시지 처리 실패의 복잡성](#1-서론-이벤트-기반-분산-시스템에서-메시지-처리-실패의-복잡성)

- [2. 비차단 재시도(Non-blocking Retry) 아키텍처 설계와 다단계 재시도 토픽 구조](#2-비차단-재시도non-blocking-retry-아키텍처-설계와-다단계-재시도-토픽-구조)

- [3. 데드 레터 큐(DLQ)의 역할과 알림 및 운영 모범 사례](#3-데드-레터-큐dlq의-역할과-알림-및-운영-모범-사례)

- [4. 아키텍처적 트레이드오프 및 설계 선택 가이��](#4-아키텍처적-트레이드오프-및-설계-선택-가이드)

- [5. 결론: 무중단 고가용성 MSA 구축을 위한 장애 대응 설계 제언](#5-결론-무중단-고가용성-msa-구축을-위한-장애-대응-설계-제언)
...
사전에 정의된 최대 재시도 임계값(예: 3회 또는 5회)을 초과하도록 예외 상황이 지속되면, 해당 이벤트를 독성 메시지(Poison Pill)로 판���하고 데드 레터 큐(DLQ) 토픽으로 최종 영구 이적시킵니다 [1], [2], [5].
