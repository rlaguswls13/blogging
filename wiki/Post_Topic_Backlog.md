...
- 관리자(사용자)가 대화 세션 중 주제 추천을 물어보거나 골라서 생성을 지시할 때, Agent는 본 백로그([`wiki/Post_Topic_Backlog.md`](file:///d:/coding-project/2026-project/ai-blogging/wiki/Post_Topic_Backlog.md))에서 **미발행(🟡)** 상태인 번호(B-3, B-5, A-1, A-3~A-5) 또는 주제를 읽어 즉시 `python main.py new --topic "주제명"` 파이프라인을 구동합니다.
