...
17. **시리즈 글은 `{키워드}_Series` 태그를 함께 부여 (Series Tag Rule)** — 2026-08-23 추가:
    - **왜**: GoF 14부작·NoSQL/RDBMS #1·#2처럼 여러 편으로 이어지는 시리즈는 `check_topic_duplication.py`가 "같은 시리즈라 제목/태그가 겹치는 게 정상"이라고 판단해 임계값을 완화해주지만(아래 18번 수칙 참고), 그건 오탐만 막아줄 뿐 "이 글들이 한 시리즈다"라는 사실 자체는 어디에도 명시적으로 남지 않는다. 나중에 시리즈 단위로 검색·필터링하려면 태그로 못박아둬야 한다.
    - **규칙**: 새 글이 기존 시리즈의 다음 편이거나(제목이 `[시리즈명] N.` / `[시리즈명 #N]` 패턴을 따름), 새 시리즈의 1편이라면, frontmatter `tags`에 `{시리즈명}_Series` 형식의 전용 태그를 반드시 추가한다(예: `GoF_Series`, `NoSQL_Series`). 시리즈명은 대괄호 접두어의 첫 단어(예: "GoF 디자인 패턴" → `GoF`, "NoSQL 깊이 읽기" → `NoSQL`)를 그대로 쓴다. 시리즈의 인덱스/총론 글(예: gof-14.md처럼 개별 편은 아니지만 시리즈를 안내하는 글)도 같은 태그를 붙인다.
    - **검증**: `python src/tools/manage_series_tags.py --scan`으로 전체 글을 다시 스캔해 빠진 시리즈 태그가 없는지 주기적으로 확인할 수 있다(제목 대괄호 접두어 매칭이 1차 신호, 클러스터 전용 태그+제목 유사도 0.35 이상이 2차 신호 — 오검출 방지를 위해 두 시리즈가 같은 태그를 공유하면 그 태그는 판별에서 제외한다).

18. **시리즈 태그가 있으면 토픽 중복 임계값을 우선 신뢰 (Series-Aware Duplication Check)** — 2026-08-23 추가:
    - **왜**: 17번 수칙으로 시리즈 태그가 명시적으로 존재하게 됐으므로, `check_topic_duplication.py`의 시리즈 판별은 이제 "비일반 태그가 겹치는가"라는 약한 휴리스틱보다 "정확히 같은 `_Series` 태그를 갖고 있는가"를 우선순위 높은 신호로 쓴다.
    - **규칙**: 새 글이 기존 시리즈의 다음 편이면 topic_selection 단계에서 `--tags`에 그 `_Series` 태그를 반드시 포함해 `check_topic_duplication.py`를 돌린다 — 그러면 임계값이 자동으로 0.9까지 완화되어(`SERIES_THRESHOLD`), 시리즈 내 정상적인 제목/태그 겹침을 중복 경고로 오판하지 않는다. `_Series` 태그 없이 비일반 태그 겹침만으로도 여전히 완화는 동작하지만(구버전 글 소급 대응용), 신규 작성 시에는 `_Series` 태그를 명시하는 쪽을 우선한다.
