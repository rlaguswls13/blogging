...
- **기존 47개 발행 글 소급 보완**: 14번 수칙(차별화 포인트)과 15번 수칙(백링크 라이브 렌더링)은 신규 작성 글부터 적용된다. 이미 발행된 글에 차별화 포인트를 소급 추가하고 boilerplate 문장을 다양화하는 재작업은 범위가 커서 별도 세션에서 우선순위 있는 글부터 순차 진행을 검토할 것.
- **`content/posts/*.md` 5개 발행 글의 `
